...
import {
  CATBOOST_PYTHON_BIN_ENV,
  CATBOOST_SCORER_SCRIPT_ENV,
  CATBOOST_SCORER_TIMEOUT_MS_ENV,
...
import {
  ...
  resolveSixFactorPolicyDecisionWithFeatures,
...
type CatBoostPolicyV2LearnerFlowSmokeResult = {
  readonly ok: true;
  readonly artifactPath: string;
  readonly defaultArtifactPathStillLinear: boolean;
  readonly pythonScorerAvailable: boolean;
  readonly catboostImportWorks: boolean;
...
function runCatBoostImportCheck(env: Record<string, string | undefined>) {
  const pythonBin = readString(env[CATBOOST_PYTHON_BIN_ENV]) ?? defaultPythonBin();
  const timeoutMs = Number.parseInt(
    env[CATBOOST_SCORER_TIMEOUT_MS_ENV] ?? "10000",
    10,
  );
  const result = spawnSync(pythonBin, ["-c", "import catboost"], {
    encoding: "utf8",
    shell: false,
  ...
  assert(
    ...
    `CatBoost import failed: ${
      result.error instanceof Error
        ...
        : String(result.stderr ?? "").trim() || "python exited non-zero"
    }`,
  );
}
...
export function runCatBoostPolicyV2LearnerFlowSmoke(
  env: Record<string, string | undefined> = process.env,
): CatBoostPolicyV2LearnerFlowSmokeResult {
  const artifactPath =
  ...
  const scorerScript = resolveRepoPath(
    readString(env[CATBOOST_SCORER_SCRIPT_ENV]) ?? DEFAULT_SCORER_SCRIPT,
  );
  assert(existsSync(scorerScript), `CatBoost Python scorer is missing: ${scorerScript}`);
  runCatBoostImportCheck(env);
  ...
  const resolved = resolveSixFactorPolicyDecisionWithFeatures(
    {
      userRef: "catboost_policy_v2_smoke_user",
      subjectRef: "mathematics",
      topicRef: "linear_equations",
      sessionRef: "catboost_policy_v2_smoke_session",
      contentEventRef: "catboost_policy_v2_smoke_event",
  ...
  assert(decision.decisionSource === "ml_policy", "CatBoost decisionSource must be ml_policy.");
  assert(
  ...
  assert(decision.fallbackUsed === false, "CatBoost learner-flow smoke must not use fallback.");
  assert(
  ...
  return {
    ...
    pythonScorerAvailable: true,
    catboostImportWorks: true,
