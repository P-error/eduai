...
Outcome-поля запрещены в feature snapshot: `pre_score`, `post_score`, `max_score`, `next_step_success`, `normalized_learning_gain`, `normalized_learning_gain_clamped`, `outcome_available`, `outcome`.
...
`outcome.normalized_learning_gain` хранит signed gain в диапазоне `[-1, 1]`: отрицательное значение означает ухудшение результата после контента. Если нужен legacy-совместимый неотрицательный показатель, используется отдельное поле `normalized_learning_gain_clamped` в диапазоне `[0, 1]`; оно не является основным signed target.
